  ...
  Future<void> _submit() async {
    ...
    try {
      if (_authMode == AuthMode.Login) {
        ...
        await Provider.of<Auth>(context, listen: false)
            .saveDoctorDate(_authData['Name'], selectedValue!,  _authData['phoneNumber'], _authData['hospital']);
       // await Provider.of<Auth>(context, listen: false).FetchDoctorDate();
        // Sign user up
      } else if (_authMode == AuthMode.PatientSignUp) {
        await Provider.of<Auth>(context, listen: false)
            .Signup(_authData['email'], _authData['password']);
        await Provider.of<Auth>(context, listen: false).savePatientDate(_authData['Name'], _authData['age'], _authData['phoneNumber']);
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Container(
        ...
        constraints:
            BoxConstraints(minHeight: 400), //_authMode == AuthMode.Login ? 300 : 700),
          ...
          child: SingleChildScrollView(
            child: Column(
              children: <Widget>[
                ...
                if (_authMode == AuthMode.Signup)
                  TextFormField(
                    ...
                    validator: _authMode == AuthMode.Signup
                        ...
                        : null,
                  ), if (_authMode == AuthMode.PatientSignUp) TextFormField(
                ...
                if (_authMode != AuthMode.PatientSignUp) ElevatedButton(
                  child: Text(
                      '${_authMode == AuthMode.Login ? 'DoctorSignUp' : 'LOGININSTEAD'} '),
                  onPressed: _switchAuthMode,
                 
                ),
                 if (_authMode != AuthMode.Signup ) ElevatedButton(
                    child: Text(
